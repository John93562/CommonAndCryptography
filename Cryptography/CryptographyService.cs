...
namespace Cryptography
{
    public class CryptographyService
    {
        ...
        public string EncryptZip(string filePath, string key)
        {
            string outputPath = "";
        ...
        public string DecryptZip(string filePath, string key, string outputPath)
        {
        ...
        public void DecryptFolderUpdated(string folderPath, string key1andcurrentprogramkey, string rootPath)
        {
            ...
            DecryptZip(FullBackupName, key1andcurrentprogramkey, temptempZip);
        ...
        public async Task EncryptFoldersUpdated(string folderPath, string key1andcurrentprogramkey, string backupTempFolder, string rootFolder)
        {
            string BackupName = $"temp";
            ...
            string outputPath = EncryptZip(FullBackupName, key1andcurrentprogramkey);
        ...
        public void EncryptFolders(string Key, string folderPath)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                do
                {
                    ...
                    try
                    {
                        ...
                        FileInfo file = new FileInfo(files[i]);
                        if (!files[i].Contains(".enc"))
                        {
                            FileEncrypt(files[i], Key);
                        }
        ...
        public void DecryptFolders(string Key, string folderPath)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        {
                            FileDecrypt(files[i], files[i].Replace(".enc", ""), Key);
        ...
        public static void DeleteAllEncryptedFilesAfterDecryptWitchMakesNoSenseToKeepTheseFilesPresentBecauseTheyMightFuckUpTheSystem(string rootFolder)
        {
            string rootFolderWithAssets = rootFolder + "\\assets";

            if (!Directory.Exists(rootFolderWithAssets))
            {
                return;
            }


            IEnumerable<string> files = Directory.GetFiles(rootFolderWithAssets, "*",
                SearchOption.AllDirectories).Where(e => e.Contains(".enc"));
        ...
        private void FileEncrypt(string inputFile, string password, string extension = ".enc")
        {
            ...
            if (!inputFile.Contains(".db"))
            {
                File.Delete(inputFile);
            }
        }

        public void EncryptFile(string fullPath12, string extension, string password)
        {
            FileEncrypt(fullPath12, password, extension);
        }

        public string DecryptFile(string fullPath12, string fullPathButPhotoPathNameFull, string password, bool deleteAfterDecrypt = false)
        {
        ...
        /// <param name="password"></param>
        private string FileDecrypt(string inputFile, string outputFile, string password, bool deleteAfterDecrypt = true)
        {
